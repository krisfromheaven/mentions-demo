...
@Component({
  selector: 'app-ngx-mentions',
  templateUrl: './ngx-mentions.component.html',
  styleUrl: './ngx-mentions.component.scss',
})
export class NgxMentionsComponent implements OnInit {
  textCtrl: FormControl = new FormControl('', { nonNullable: true });
  loading = false;
  choices: User[] = [];
  ...
  ngOnInit() {
    this.textCtrl.valueChanges.subscribe(
      (content) =>
